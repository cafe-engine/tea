...
int main(int argc, char ** argv) {
  ...
  while (!tea_should_close()) {
    ...
    if (tea_keyboard_was_pressed(TEA_KEY_A)) x -= 10;
    else if (tea_keyboard_was_pressed(TEA_KEY_D)) x += 10;

    tea_set_canvas(canvas);
    tea_render_clear(BLACK);
    tea_draw_image(img, &tea_rect(0, 0, 16, 16), tea_point(x, y));
    tea_draw_color(WHITE);
    tea_draw_rect(tea_rect(x, y, 16, 16));
    tea_set_canvas(0);
